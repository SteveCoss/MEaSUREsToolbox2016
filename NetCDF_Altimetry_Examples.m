...
addpath 'C:\Users\Public\Documents\coss.31\Documents\MATH\Steves_final_Toolbox\MEaSUREsToolbox2016\AltimetryToolbox\Dataproduct' % change this to the local directory where you have the altimetry data
...
for i=1:length(Rivers),
 ...
 formatSpec = '%s';
    ...
    fclose(FID);
    VSLIST=dataArray{1};
    if ~isempty(VSLIST)&& length(char(VSLIST(1)))<40;
...
for i=1:length(Rivers)
    ...
    for j=1:length(VSLIST),
        ...
        var4=ncread(FNAME2,'/Unprocessed GDR Data/lon');
        var5=ncread(FNAME2,'/Unprocessed GDR Data/lat');
        var6=ncread(FNAME2,'/Unprocessed GDR Data/h');
        var3=var3';
        gdat=(h>-1);
...
tthaw=ncread(FNAME2,'/Filter/icethaw');
tthaw=tthaw+startdate;
tthaw=datevec(  tthaw);
tfreeze=ncread(FNAME2,'/Filter/icefreeze');
tfreeze=tfreeze+startdate;
tfreeze=datevec(tfreeze);
